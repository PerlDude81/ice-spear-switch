...
module.exports = class Mubin_Editor {
    ...
    constructor(canvasNode, uiNode, project, loader, stringTable = undefined) {
        ...
        this.mubinRenderer = new Mubin_Renderer(
            canvasNode,
            uiNode,
            this.loader
        );
        ...
        this.actorEditor = new Actor_Editor(
            this.mubinRenderer,
            this.actorHandler
        );
        this.rendererSettings = new Renderer_Settings(this, uiNode);

        this.actorLoader = new Actor_Loader(this.actorHandler, actorType =>
            this.generateMubinPath(actorType)
        );
        this.prodLoader = new PROD_Loader(this.actorHandler, actorType =>
            this.generateProdPath(actorType)
        );

        this.actorFinder = new Actor_Search_Finder(
            this.actorHandler,
            this.actorEditor
        );
    ...
    testFreeHashesFileExists() {
        var filePath = process.cwd()+"/free-hashes.txt";

	// does file exist?
        if (fs.existsSync(filePath)) {

		// still some hash ids ( > 500)
		var freeData = fs.readFileSync(filePath, 'utf8');
		if (freeData.toString().split('\n').length < 500) {

            		swal({
            		    title: "You're running out of hashes!",
            		    html:
            		        "You're seeing this because the free-hashes.txt file you're using has fewer than 500 hash ids left.\n Please ask for a new file in the team discord.",
            		    type: "error"
            		});
			return;
            return;
            ...
            swal({
                title: "Missing free-hashes.txt file!",
                html:
                    "Please make sure to have the free-hashes.txt file in the same folder where the ice-spear exe is located. ",
                type: "error"
            });
            return;
        }
	...
	this.testFreeHashesFileExists();

        this.mubinDir = directory;
	// Added this check
        this.mubinName = (name) ? name : 'FinalTrial' ;
        this.actorHandler.loader = this.loader;

	this.actorHandler.setMubinStuff(directory,name);
        ...
        if (this.loadActorData) {
            if (this.loader)
                await this.loader.setStatus("Loading Mubin-Actors");
            await this.actorLoader.load(this.mubinDir, this.mubinName);
        }

        if (this.loadProdData) {
            if (this.loader)
                await this.loader.setStatus("Loading PrOD-Objects");
