...
module.exports = class Renderer_Settings {
    ...
    _addEvents() {
        ...
        actorVisibleNode.onchange = () =>
            this.mubinEditor.showVisibleActors(actorVisibleNode.checked);

        const actorInvisibleNode = this._getToolNode("actorsInvisible");
        actorInvisibleNode.onchange = () =>
            this.mubinEditor.showInvisibleActors(actorInvisibleNode.checked);

        const actorFarLODNode = this._getToolNode("actorsFarLOD");
        actorFarLODNode.onchange = () => {
            const actorGroup = this.mubinEditor
                .getRenderer()
                .scene.getObjectByName("actors");
            if (!actorGroup || !actorGroup.children) return;
        ...
        mapModelNode.onchange = () =>
            this.mubinEditor.showMapModel(mapModelNode.checked);

        const accurateTimerNode = this._getToolNode("accurateTimer");
        accurateTimerNode.onchange = () =>
            this.mubinEditor.mubinRenderer.useAccurateTime(
                accurateTimerNode.checked
            );

        const postProcNode = this._getToolNode("postProc");
        postProcNode.onchange = () =>
            this.mubinEditor
                .getRenderer()
                .usePostProcessing(postProcNode.checked);

        const camLightNode = this._getToolNode("camLight");
        camLightNode.onchange = () => {
            this.mubinEditor.getRenderer().helper.lighting.cameraLight.visible =
                camLightNode.checked;
        };

        const showStatsNode = this._getToolNode("showStats");
        showStatsNode.onchange = () =>
            this.mubinEditor.getRenderer().useStats(showStatsNode.checked);

        const targetFpsNode = this._getToolNode("targetFPS");
        targetFpsNode.onchange = () =>
            this.mubinEditor
                .getRenderer()
                .setTargetFPS(parseInt(targetFpsNode.value));

        const camSpeedNode = this._getToolNode("camSpeed");
        camSpeedNode.onchange = () => {
            this.mubinEditor.getRenderer().helper.fpsControls.camSpeed =
                camSpeedNode.value;
        };

        const boostSpeedNode = this._getToolNode("boostSpeed");
        boostSpeedNode.onchange = () => {
            this.mubinEditor.getRenderer().helper.fpsControls.boostSpeed =
                boostSpeedNode.value;
        };
