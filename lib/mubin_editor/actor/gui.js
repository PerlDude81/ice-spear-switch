...
module.exports = class Actor_GUI {
    ...
    update() {
        ...
        this.nodeCache.get(".data-actor-unique-name").innerHTML = this.actor.getUniqueName();
        ...
        this.nodeCache.get(".data-actor-params-HashId").value =
            this.actor.params.HashId.value;
        this.nodeCache.get(".data-actor-params-SRTHash").value =
            this.actor.params.SRTHash.value;

        this.nodeCache.get(".data-actor-params-FieldCRC32").innerHTML =
            this.actor.getCRC32("MainField");

        const modelTypeImg = this.actor.isInvisible()
            ? "with-model.png"
            : "no-model.png";
        this.nodeCache
            .get("img.actor-model-type")
            .setAttribute("src", "assets/img/icons/actor/" + modelTypeImg);
    ...
    _addCallbacks() {
        ...
        this._addClickEvent("tool-deselect", () =>
            this.actor.handler.deselectActor(this.actor)
        );
        this._addClickEvent("tool-focus", () =>
            this.actor.handler.focusActor(this.actor)
        );
        ...
        this._addClickEvent(
            "tool-addRotScalar",
            () => this.actor.addRotationParams(false),
            true
        );
        this._addClickEvent(
            "tool-addRotVector",
            () => this.actor.addRotationParams(true),
            true
        );

        this._addClickEvent(
            "tool-addUniqueName",
            () => this.actor.addUniqueName(true),
            true
        );
        ...
        this._addClickEvent(
            "tool-addScaleScalar",
            () => this.actor.addScalingParams(false),
            true
        );
        this._addClickEvent(
            "tool-addScaleVector",
            () => this.actor.addScalingParams(true),
            true
        );

        this._addClickEvent(
            "tool-removeRot",
            () => this.actor.removeRotationParams(),
            true
        );
        this._addClickEvent(
            "tool-removeScale",
            () => this.actor.removeScalingParams(),
            true
        );

        this._addClickEvent("tool-copyJSON", () =>
            clipboard.writeText(this.actor.getParamJSON())
        );

        this._addClickEvent(
            "tool-importJSON",
            () => this._importJSON(clipboard.readText(), true),
            true
        );
    ...
    async _importJSON(jsonString, keepPosition) {
        ...
        try {
            ...
            swal({
                title: "Import failed!",
                html:
                    "Undo a few steps to avoid a potentially broken state.\nException: " +
                    e,
                type: "error"
            });
        }
    ...
    showEditor() {
        if (this.editor) this.hideEditor();
    ...
    hideEditor() {
        ...
        this.editorNode.parentNode.replaceChild(
            this.editorNode.cloneNode(false),
            this.editorNode
        );
        this.editorNode = this.node.querySelector(".byaml-editor");
    }
