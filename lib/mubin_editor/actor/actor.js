...
module.exports = class Actor {
    ...
    getName() {
        return this.params.UnitConfigName && this.params.UnitConfigName.value
            ? this.params.UnitConfigName.value
            : "";
    }

    getHashId() {
        return this.params.HashId && this.params.HashId.value
            ? this.params.HashId.value
            : 0;
    }
    ...
    getSRTHash() {
        return this.params.SRTHash && this.params.SRTHash.value
            ? this.params.SRTHash.value
            : '';
    }

    getUniqueName() {
        return this.params.UniqueName && this.params.UniqueName.value
            ? this.params.UniqueName.value
            : '';
    }
    ...
    copy() {
        if (this.handler) this.handler.copyActor(this);
    }
    ...
    async importParamJSON(jsonString, keepPosition = false) {
        ...
        if (
            ...
            newParams.HashId == null
        ) {
            swal({
                title: "JSON is invalid!",
                html:
                    "Make sure your clipboard contains a valid actor JSON.\nContent: " +
    ...
    async updateObject() {
        if (this.handler) {
            ...
            this.objInstance =
                await this.handler.actorObjHandler.createInstance(
                    this.getName()
                );
            this.objInstance.setActor(this);
    ...
    async delete() {
        if (this.gui) this.gui.delete();

        if (this.handler) await this.handler.deleteActor(this);
    ...
    // default to this.params.HashId.value
    addUniqueName() {

	if (this.params.UniqueName && this.params.UniqueName.value) {
		// do nothing, UniqueName exists
	} else {

		this.params.UniqueName = BYAML.Helper.fromJSON('{"type": 160, "value": "'+this.params.HashId.value+'"}'
	            );
