...
module.exports = class App extends App_Base {
    constructor(window, args) {
        ...
        this.selectShrine.ondblclick = function (a, b) {
            if (this.value != null)
                that.windowHandler.open("shrine_editor", {
                    shrine: this.value
                });
        };
        ...
        this.shrineFilter = new Filter(
            this.node.querySelector(".shrine input"),
            this.selectShrine,
            "option",
            null
        );
        this.modelFilter = new Filter(
            this.node.querySelector(".model  input"),
            this.selectModel,
            "option",
            null
        );
    ...
    async scanShrineDir() {
        ...
        let files = await readdir(shrineDir);
        if (!files) return;

        let aocShrineDir =
            this.config.getValue("game.aocPath") + "/content/0010/Pack";

        const files2 = await readdir(aocShrineDir);
    ...
    async scanModelTextureDir() {
        let modelDir =
            this.config.getValue("game.updatePath") + "/content/Model";

        fs.readdir(modelDir, (err, files) => {
            if (files == null) return;

            let secondModelDir =
                this.config.getValue("game.basePath") + "/content/Model";

            fs.readdir(secondModelDir, (err, files2) => {
                if (files == null) return;

                this.selectModel.innerHTML = files
                    .concat(files2)
                    .reduce((modelsHtml, file) => {
